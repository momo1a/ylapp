...
class UeMsg
{
    ...
    public function __construct(){
        ...
        $this->CI->load->config('ue_msg');
        $this->_spId = $this->CI->config->item('ue_spid') or ajax_error('缺少配置项：ue_spid');
        $this->_password = $this->CI->config->item('ue_password') or ajax_error('缺少配置项：ue_password');
        $this->_accessCode = $this->CI->config->item('ue_accessCode') or ajax_error('缺少配置项：ue_accessCode');
        $this->_tag = $this->CI->config->item('ue_tag') or ajax_error('缺少配置项：ue_tag');
    }
     ...
     * @param $timeLen
     */
    public function sendMsg($mobile,$code,$timeLen){
        $content = '【'.$this->_tag.'】尊敬的用户你好，你的验证码为'.$code.'验证码有效期为'.$timeLen.'秒。';
        try {
            $client = new soapclient('http://113.105.65.138:7801/sms?WSDL', array('encoding' => 'utf-8'));

            $res = $client->Submit($this->_spId, $this->_password, $this->_accessCode, $content, $mobile);
            return substr($res,0,1);

        } catch (SoapFault $fault) {
            exit(json_decode(array('code'=>$fault->faultcode,'msg'=>$fault->faultstring)));
        }
    }
}