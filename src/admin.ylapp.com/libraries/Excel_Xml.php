...
class Excel_Xml
{
    ...
    /**
     * 
     * @param string $filename Filename to use in download dialog save
     */
    public function download($filename)
    {
        $filepath = APPPATH.'/cache/excel_'.time();
        $this->generateWorkbook();
        $this->_getSanitizedWorkbookTitle($filename);
        if (!$handle = @fopen($filepath, 'w+')) {
            throw new Exception(sprintf("Not allowed to write to file %s", $filepath));
        }
        if (@fwrite($handle, $this->sOutput) === false) {
            throw new Exception(sprintf("Error writing to file %s", $filepath));
        }
        @fclose($handle);
