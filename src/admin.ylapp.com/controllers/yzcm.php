 ...
 */
class Yzcm extends MY_Controller
{
	/**
	 * 要放入视图的数据
	 * @var array
	...
	public function __construct() {
		...
		$this->load->model('goods_yzcm_model');
	...
	public function index($type = 'ongoing') {
		...
		// 分页偏移量
		$this->goods_yzcm_model->offset = $this->uri->segment(3);
		
		$this->view_data['list'] = $this->goods_yzcm_model->find_yzcm_limit($type, $where);

		// HTML分页设置
		$page_conf = array('uri_segment'=>3, 'anchor_class'=>'type="load" rel="div#main-wrap"');
		$page_conf['base_url'] = site_url('yzcm/'.$type);
		$page_conf['first_url'] = site_url('yzcm/'.$type);
		
		$this->view_data['pager'] = $this->pager($this->goods_yzcm_model->total_count, $this->goods_yzcm_model->limit, $page_conf);
		$this->view_data['get_type'] = $type;
		
		$this->load->view('yzcm/index', $this->view_data);
	...
	public function sort() {
		...
		if ($sorts) {
			$this->goods_yzcm_model->update_manual_sort($sorts);
		}
