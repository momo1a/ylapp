 ...
 * 一站成名的商品数据处理model
 * 
 * 我希望model的方法能纯粹一些,避免因为方法公共过多,方法一修改(需求变更)导致的BUG
 * 如果程序按模块划分,那起的model名应该是goods_model
 * 
 * @author "韦明磊<nicolaslei@163.com>"
 *
 */
class Goods_yzcm_model extends MY_Model
{
	/**
	 * 活动表名
	 * @var string
	 ...
	 * @return array
	 */
	public function find_yzcm_limit($get = 'complete', $wheres = NULL) {
		...
		// 一站成名的活动
		$this->db->where('type', self::$type_yzcm);
