...
var mpg = {
	...
	refund:function(o){
		...
		var dia   = art.dialog({
            title  : "申请退款",
            content: "诚信金退还后，您也可以正常发布【名品馆】活动。",
            icon   : "question",
            lock   : true,
            cancel : true,
            button : [
                {
                    ...
                    callback: function(){
            			...
            			var _tips_dialog = mpg.do_submit('正在操作，请稍后...', 120, function() {
            				$.post('/mpg/undeposit/', {doundeposit:1,inajax:1},function(ret){
            					_tips_dialog.close();
            					ret = $.parseJSON(ret);
            					if(ret.success){
