...
class User_center extends MY_Controller
{
    ...
    public function onlineAskDetail(){
        ...
        $res = $this->online_ask->getDetailById(self::$currentUid,$id,'id,askNickname as name,docId,docName,askTelephone,FROM_UNIXTIME(hopeCalldate) AS hopeDate,phoneTimeLen as timeLen,price,askContent,(case when state=0 then "未支付" when state=1 then "待处理" when state=2 then "已确认沟通时间" when state=3 then "完成" when state=4 then "失败" end) as state,docRemark');
        $hosAddr = $this->user->getDoctorDetail($res['docId'],'YL_hospital.address as hosAddr,YL_hospital.name as hosName,YL_doctor_offices.officeName');
        $res['hosAddr'] = $hosAddr[0]['hosAddr'];
        $res['officeName'] = $hosAddr[0]['officeName'];
        $res['hosName'] = $hosAddr[0]['hosName'];
        $this->response($this->responseDataFormat(0,'请求成功',array($res)));
    }
    ...
    public function appointDetail(){
        ...
        $hosAddr = $this->user->getDoctorDetail($res['docId'],'YL_hospital.name as hosName,YL_hospital.address as hosAddr,YL_doctor_offices.officeName');
        $res['hosAddr'] = $hosAddr[0]['hosAddr'];
        $res['officeName'] = $hosAddr[0]['officeName'];
        $res['hosName'] = $hosAddr[0]['hosName'];
        $this->response($this->responseDataFormat(0,'请求成功',array($res)));
    ...
    public function updatePayPassword(){
        $inputOriPwd = trim($this->input->get_post('oriPwd'));
        $payPwd = trim($this->input->get_post('payPwd'));
        $rePayPwd = trim($this->input->get_post('rePayPwd'));
        if(!self::$currentUid){
            $this->response($this->responseDataFormat(2,'用户请求异常',array()));
    ...
    public function msgList(){
        ...
        $msgFo = $this->order->getOrdersMsg(self::$currentUid,'oid as id,packageId as gid,packageTitle,from_unixtime(dateline) as dateline,(case when status=1 then "待支付" when status=2 then "已支付" when status=3 then "待处理" when status=4 then "已通知" when status=5 then "完成" end) as state,(case when type=1 then "疫苗接种" when type=2 then "检测服务" end) as msgType');
