  ...
  <div class="ui-box-outer">
    <div class="ui-box-inner">
      <ul class="ui-tab-nav" id="tabs">
        <li class="ui-tab-item ui-tab-itemCurrent"><a href="<?php echo site_url('cash/index/cash_send');?>">手动发放</a></li>
      ...
      <div class="ui-box ui-box2 advertisement-add">
        ...
        <form id="cash_send_form" method="post" action="<?php echo site_url('cash/hand_send')?>" enctype="multipart/form-data">
          <ul>
	        <li><span>现金券类型:</span>
...
<?php endforeach; endif;?>
            ...
            <li><span>发放对象:</span>
            <textarea data-type="users" name="cash[users]" placeholder="用户名用英文逗号隔开，例如 xiao123,resd34,芙蓉" class="ui-form-text" style="width: 400px;height: 80px" data-rule="required" data-msg="*请输入发放用户名"></textarea>
            <span id="for_cash_users_" style="color: red; width: 111px;"></span>
            </li>
            <li>
    ...
    !function($form){
        ...
        if( cash_id ){
            $('#cash_condition').load('<?php echo site_url('cash/show_cash/').'/';?>'+$('#cash_cid').val(), function(){
                $form.find("[data-type=users]").blur();
            });
        }
        ...
        $form.find("[data-type=users]").blur(function(){
            var ss = $.trim($(this).val()).replace(/，/g,",").replace(/^,|,$/g, "").replace(/,(\s*,)+/,","),
                lh = ss ? ss.split(",").length : 0,
