var Check = {
	...
	getMsg : function(s){
		var d={
			NAME_LENGTH_OUT      : "用户名为6-50个字符，1个汉字为2个字符。推荐使用中文用户名。",
...
//ajax请求验证
Check.ajax = function(data){
	$.ajax({
	    url: '/reg/check/',
	    type: "post",
	    data:data,
	    dataType: "json",
	    error: function(){Check.getMsg("NET_ERROR")},
	    success: function(data) {
	    	if(!data.success)
				Check.getMsg(data.msg);
	    }
	});
};
//用户名
Check.name = function(str){
	if(this._lengthAt(str)<6 || this._lengthAt(str)>50){
		return Check.getMsg("NAME_LENGTH_OUT");
	}

	if(/\d{5}/.test(str)){
		return Check.getMsg("NAME_MORE_NUMBER");
	}
	return true;
};
//手机号
Check.mobile=function(str){
	if(!/^1\d{10}$/.test(str))
		return  Check.getMsg("MOBILE_FORMAT_WRONG");
	return true;
...
//		return  Check.getMsg("EMAIL_FORMAT_WRONG");
//	//this.ajax({'account':str,'method':'check_email'}, callback);
//};
//密码
Check.password = function(str){
	str = str.toLowerCase();
    if(!str)
        return  Check.getMsg("PWD_IS_NULL");
    if(/^[a-z]+$/.test(str))
        return  Check.getMsg("PWD_ALL_LETTER");
    if(/^\d+$/.test(str)) 
    	return  Check.getMsg("PWD_ALL_NUMBER");
    if(/^_+$/.test(str))
        return  Check.getMsg("PWD_ALL_SYMBOL");
    if(!/^[_0-9a-z]{6,20}$/.test(str))
        return  Check.getMsg("PWD_FORMAT_WRONG");
/*    // 不允许4个连续递增的数字或字母
...
//验证码验证函数
Check.captcha = function(str){
	if(!str)
		return  Check.getMsg("CAPTCHA_IS_NULL");
    if(str.length<4)
    	return  Check.getMsg("CAPTCHA_IS_WRONG");
