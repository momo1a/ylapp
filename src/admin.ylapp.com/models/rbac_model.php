...
class Rbac_model extends CI_Model{
	...
	*/
	public function get_acl($role_id){
		...
		//if($data['role_id']>0){
		$query = $this->db->query("SELECT id,dirc,cont,func FROM `YL_node` WHERE id in (SELECT node_id FROM `YL_auth` WHERE role_id = ".$role_id.")");
	...
	*/
	public function check_user($username,$password){
		$query = $this->db->query("SELECT uid,password,nickname,email,role_id,status FROM `YL_user` WHERE nickname = '".$username."' LIMIT 1");
		$data  = $query->row_array();
		if($data){
			if($data['status']==1){
